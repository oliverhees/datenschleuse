...
Gemessen wird gegen einen eigenen deutschen Testkorpus (82 Fälle, `test/corpus/`), jederzeit selbst wiederholbar: `python3 test/corpus-benchmark.py` misst den ausgelieferten Zustand, `--stopwords presidio/de-stopwords.yml` zusätzlich den mit der Nicht-PII-Wortliste erreichbaren.
...
Die rechte Spalte ist eine geprüfte, aber **nicht ausgelieferte** Fähigkeit. Es gibt eine gemessene Nicht-PII-Wortliste (`presidio/de-stopwords.yml`), die deutsche Alltagswörter in ASCII-Umschrift und Schema-Schlüssel wie `bestellnummer` von der Fehlalarmierung ausnimmt — ohne einen einzigen Recall-Verlust: 54 von 54 Pflicht-Entitäten werden in beiden Läufen gefunden. Sie liegt im Repo, sie ist gegen den laufenden Analyzer belegt, und sie wirkt im Benchmark. **Nur sendet der Guardrail sie nicht an Presidio** — im Proxy ist sie schlicht nicht angeschlossen. Der nächste Schritt ist deshalb kein Erkennungsproblem, sondern eine Verdrahtung: Die nötige Änderung ist in [`docs/foundation/erkennungsziel.md`](docs/foundation/erkennungsziel.md) §7 spezifiziert („Spezifikation, nicht umgesetzt") und ist ein eigenes Work Item. Warum es die Liste überhaupt gibt, welche vier Gegenkontrollen sie einhegen und welche Alternativen nach Messung verworfen wurden: [ADR-0002](docs/adr/0002-nicht-pii-wortliste.md).
...
**Das Erkennungsziel ist damit nicht erreicht, und es wird nicht abgesenkt, bis es passt.** Precision und Störquote stehen beide rot, und sie stehen aus einem benannten Grund rot: ASCII-umschriebene Alltagswörter (`Aendere`, `Spaeter`) erzeugen denselben Span wie ein echter Nachname. Die Wortliste sieht nur diesen Span — sie kann den Fehlalarm nicht unterdrücken, ohne den Namen mitzunehmen. Das braucht einen kontextsensitiven Recognizer und ist offen. Der Benchmark-Runner liegt im Repo, die Messung wird gefahren statt geschätzt, und `test/test_erkennungsziel_gate.py` setzt die Schwellen maschinell gegen das Zielpapier durch. Ein Ziel ohne maschinelle Durchsetzung ist ein Wunsch, den man beim nächsten roten Lauf stillschweigend anpasst.
